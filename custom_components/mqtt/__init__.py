...
class MqttAvailability(Entity):
    ...
    async def _availability_subscribe_topics(self):
        ...
        def availability_message_received(msg: Message) -> None:
            ...
            _LOGGER.info("+++++++++++++++++++++++++msg.payload={}".format(msg.payload))
#             _LOGGER.info("+++++++++++++++++++++++++self._avail_config[CONF_PAYLOAD_AVAILABLE]={}".format(self._avail_config[CONF_PAYLOAD_AVAILABLE]))
#             _LOGGER.info("+++++++++++++++++++++++++type={}".format(type(self._avail_config[CONF_PAYLOAD_AVAILABLE])))
            json_payload = json.loads(msg.payload)
#             _LOGGER.info(json_payload)

            if json_payload['avail'] == self._avail_config[CONF_PAYLOAD_AVAILABLE]:
                _LOGGER.info("avail")
                self._available = True
            elif json_payload['avail']== self._avail_config[CONF_PAYLOAD_NOT_AVAILABLE]:
                _LOGGER.info(" not avail")
                self._available = False
