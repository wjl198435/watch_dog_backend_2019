...
class Doods(ImageProcessingEntity):
    ...
    def process_image(self, image):
        ...
        # Run detection
        start = time.time()
        _LOGGER.debug("process_image：%s image [width x height]=[%s x %s]", start, img_width, img_height)
        ...
        _LOGGER.debug("process_image：%s doods response: %s", start, response)
        return
        _LOGGER.debug(
